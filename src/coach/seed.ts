...
export function coachSeed(): CoachData {
  const clients: Omit<CoachClient, 'program' | 'status' | 'age' | 'sex' | 'heightCm' | 'proposalPending' | 'proposedDays' | 'proposalNote'>[] = [
      {
        id: 'alex',
        name: 'Alex Rivera',
        goal: 'cut',
  ...
  return {
    ...
    clients: clients.map(c => ({ ...c, program: DEFAULT_PROGRAM, status: 'active' as const, age: 28, sex: 'Male', heightCm: 178, proposalPending: false, proposedDays: null, proposalNote: null })),
  }
}
